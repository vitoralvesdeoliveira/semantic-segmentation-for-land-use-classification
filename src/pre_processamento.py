...
class MaskPreProcessor(Processor):
...
def run_preprocessing_pipeline(input_dir: str, output_dir: str, target_size: int, mask_color_map : dict):
    ...
    for image,label in tqdm(zip(image_filenames, label_filenames), total=len(image_filenames), desc="Processando Arquivos"):
        image_input_path = os.path.join(input_dir, "lotes-png",image)
        label_input_path = os.path.join(input_dir, "labels-png",label)
        
        # 2a. Leitura dos arquivos
        original_image = cv.imread(image_input_path)
        my_mask_bgr = cv.imread(label_input_path)
        
        if (original_image is None) or (my_mask_bgr is None):
            logging.warning(f"Não foi possível ler a imagem ou label.")
            continue
        ...
        if (processed_image is None) and (processed_class_mask is None):
            logging.warning(f"Não foi possível processar a imagem ou label.")
            continue
